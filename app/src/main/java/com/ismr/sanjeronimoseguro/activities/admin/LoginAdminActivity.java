...
public class LoginAdminActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mDialog = new SpotsDialog.Builder().setContext(LoginAdminActivity.this).setMessage("Espere un momento").build();
                ...
                mAuth.signInWithEmailAndPassword(email, password).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
                    ...
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        if(task.isSuccessful()){
                            ...
                            Intent intent = new Intent(LoginAdminActivity.this, MapAdminActivity.class);
                            //para que el usuario una vez que entre a la actividad de mapa ya no pueda volver atras con el boton
                            intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);
                            startActivity(intent);

                            //Toast.makeText(LoginAdminActivity.this, "el login se realizo exitosamente",Toast.LENGTH_SHORT).show();
                        }
                        else{
                            Toast.makeText(LoginAdminActivity.this, "contrasena incorrecta",Toast.LENGTH_SHORT).show();
                        }
                        mDialog.dismiss();
                    }
                });
